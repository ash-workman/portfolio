...
# Google was my main source of help for this assignment (yay Stack Overflow!), as was the Ruby documentation.

require 'mechanize'
require 'uri'

$web_crawler = Mechanize.new #where the magic lives
$web_crawler.user_agent = 'IUB-I427-michfish' #it'sa me, the web robot
...
def dfs_crawl(max_depth, startpage, output_dir, datafile)
	c_page_num = 0
	...
	while c_page_num < max_depth do
		if c_page_num == 0
			...
			if visited.include?(c_page.uri.to_s)
...
(seed_url, max_pages, output_dir) = ARGV
...
dfs_crawl(max_num, seed_url, output_dir, dat_file)
